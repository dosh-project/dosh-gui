...
bool WalletApplication::init() {
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] DOSH wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "DOSH wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    if (m_splash != nullptr) {
      m_splash->show();
      m_splash->showMessage(QObject::tr("v2.0 - Loading blockchain..."), Qt::AlignLeft | Qt::AlignBottom, Qt::white);
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have doshd with non-default RPC port, you should terminate it and relaunch LuKaWallet\n"
      "or\n"
