...
const char DEFAULT_WALLET_FILE_NAME[] = "dosh.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("doshwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("doshwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("doshwallet.desktop");
  ...
  res = autorunSettings.contains("SpesWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("SpesWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.dosh.doshwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=DOSH Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("SpesWallet", appPath);
    } else {
      autorunSettings.remove("SpesWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\dosh", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:dosh");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\dosh\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\dosh\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("doshwallet.cfg"));
